...
class Trainer():
    ...
    def __init__(self, params, world_rank):
        ...
        self.precip = True if "precip" in params else False # only for precip confg
    ...
    def validate_one_epoch(self):
                        ...
                        save_image(
                            torch.cat(
                                (gen_step_one[0, 0],
                                 ...
                                 tar[0, 0]), axis=1),
                            self.params['experiment_dir'] + "/" + str(i) + "/" + str(self.epoch) + ".png")
                    else:
                        save_image(
                            torch.cat(
                                (gen[0, 0],
        ...
        if self.precip:
            ...
            try:
                ...
                logs = {'valid_l1': valid_buff_cpu[1], 'valid_loss': valid_buff_cpu[0],
                        'valid_rmse_u10': valid_weighted_rmse_cpu[
                            0]}  # , 'valid_rmse_v10': valid_weighted_rmse[1]}
    ...
    def load_model_wind(self, model_path):
        ...
        if dist.is_initialized():
            ...
            for key in checkpoint[model_key].keys():
                if 'module.' in key: # model was stored using ddp which preprends module
                    name = str(key[7:])
    ...
    def save_checkpoint(self, checkpoint_path:str, model=None):
        if not model:
            model = self.model
